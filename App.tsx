...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedSettings) {
      ...
      setShowLanding(false); // If already have settings, skip landing
    }
  ...
  // First state: Landing Page
  if (showLanding && !settings) {
    return <Landing onStartApp={() => setShowLanding(false)} />;
  }

  // Second state: Onboarding
  if (!settings) {
    return <Onboarding onComplete={handleSettingsSave} />;
  }

  // Third state: Main App UI
  return (
